...
class MaziiWebDialog(QDialog):
    ...
    def _show_web_content(self):
        ...
        try:
            ...
            self._show_import_error_screen()
    ...
    def _show_import_error_screen(self):
        """Hiển thị màn hình báo lỗi khi chưa cài WebEngine."""
        err_widget = QWidget()
        err_widget.setStyleSheet("background-color: #FEF2F2;")
        ...
        lbl.setFont(QFont("Arial", 56))

        lbl_title = QLabel("Thiếu thư viện PyQt6-WebEngine")
        lbl_title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        lbl_title.setFont(QFont("Arial", 18, QFont.Weight.Bold))
        lbl_title.setStyleSheet("color: #991B1B;")

        lbl_sub = QLabel("Chạy lệnh sau để cài đặt:\n\npip install PyQt6-WebEngine")
        lbl_sub.setAlignment(Qt.AlignmentFlag.AlignCenter)
        lbl_sub.setFont(QFont("Courier New", 13))
        lbl_sub.setStyleSheet("color: #374151; background-color: #FEE2E2; padding: 12px; border-radius: 8px;")

        layout.addWidget(lbl)
        layout.addWidget(lbl_title)
        layout.addWidget(lbl_sub)
        self.main_layout.addWidget(err_widget)
    ...
    def _retry(self):
        ...
        if is_connected():
            ...
            # Vẫn chưa có mạng, không làm gì (user thấy màn hình offline)
            pass
