"""
grammar_hub_view.py – Menu hub Ngữ Pháp (gồm 3 nút chức năng).
"""
...
class GrammarHubView(QWidget):
    go_home = pyqtSignal()
    go_grammar_session = pyqtSignal()    # → GrammarSessionView
    go_quiz_direct = pyqtSignal()        # → GrammarSessionView (chế độ quiz)
    go_grammar_drill = pyqtSignal()      # → DrillView
    ...
    def _build_ui(self):
        ...
        buttons = [
            ("📖  Học Ngữ Pháp theo bài",       "BtnStart", self.go_grammar_session),
            ("📋  Làm bài tập Drill ngữ pháp N2","BtnHard",  self.go_grammar_drill),
        ]
